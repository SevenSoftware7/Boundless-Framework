...
public static class LoadableExtensions {
	public static void BackingFieldLoadUnload<T>(this T loadable, ref bool backingField, bool value) where T : Node, ILoadable {
		if (loadable.IsInitializationSetterCall()) {
			backingField = value;
			return;
		}
		loadable.LoadUnload(value);
	}
}